...
Use `detectStatementImportFieldMapping` or `listStatementImportFieldMappingPresets` before CSV imports when the statement shape is standard. Auto-detected imports store `autoDetected`; preset-based imports store the resolved field names plus `presetId` in the import history.
