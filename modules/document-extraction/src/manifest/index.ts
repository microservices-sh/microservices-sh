export const manifest = {
  ...
  status: "draft",
  class: "core",
  summary: "Local-first scanned document extraction: file references, OCR/LLM drafts, schema normalization, source evidence, human review, and governed Gemma/AI fallback.",
  runtime: {
    language: "typescript",
  ...
  resources: [
    { type: "d1", binding: "DB", tables: ["document_extraction_jobs"] }
  ],
  permissions: [
  ...
  connections: {
    ...
    hookPoints: {
      beforeExtractionJobCreate: { kind: "filter", scope: "document-extraction.extend" },
      beforeExtractionDraftSubmit: { kind: "filter", scope: "document-extraction.extend" },
      afterExtractionReviewed: { kind: "observer", scope: "document-extraction.observe" }
    },
    provides: { hooks: [] }
  },
  customization: {
  ...
  surfaces: {
    ...
    agentic: {
      ...
      skillPaths: ["skills/document-extraction-operator/SKILL.md"],
      approvalRequiredFor: [
        "document-extraction.reviewExtraction",
        "document-extraction.normalizeExtraction",
  ...
  skills: [
    {
      id: "document-extraction-operator",
      path: "skills/document-extraction-operator/SKILL.md",
      recommendedFor: ["admin-operations", "document-intake", "data-migration", "provider-costs"]
    }
  ],
  approval: {
    risk: "medium",
    requiresApprovalFor: ["migrations", "pii-fields", "model-download", "ai-provider-calls", "production-deploy", "external-side-effects"]
  }
} as const;
