...
const DEFAULT_TEMPLATE_ID = "client-portal-sveltekit";
...
function checkResponse() {
  const checks = [
    ...
    { id: "config", status: existsSync("microservices.config.json") ? "pass" : "fail" },
    ...
    { id: "layout", status: existsSync("src/routes/+layout.svelte") ? "pass" : "fail" }
  ];
  const failed = checks.filter((check) => check.status === "fail");
  return {
    ok: failed.length === 0,
    data: {
      template: manifest.id ?? DEFAULT_TEMPLATE_ID,
      checks
    },
    ...(failed.length
      ? {
...
function localSetup(flags) {
  ...
  if (existsSync("migrations")) {
    steps.push({ id: "local:migrate", command: "wrangler", args: ["d1", "migrations", "apply", "DB", "--local"] });
  }

  return runSteps(steps, flags, ["Run microservices local dev to start the portal locally."]);
}
...
function deploymentInput(flags, environment = "preview") {
  ...
  return {
    templateId: manifest.id ?? config.template ?? DEFAULT_TEMPLATE_ID,
    modules: moduleIds,
    config: {
      ...config,
      appName: appSlug,
      appSlug,
      template: config.template ?? manifest.id ?? DEFAULT_TEMPLATE_ID
    },
    environment,
...
function deployProvisionPlan(deploymentId, flags) {
  return {
    ...
    data: {
      ...
      sideEffects: [
        "ask the control-plane API to create or reuse deployment D1/R2 resources",
        "store resource ids on the deployment resource records"
      ],
      notDoneLocally: [
        "no local wrangler d1 create",
        "no local wrangler r2 bucket create",
...
function deployCleanupPlan(deploymentId, flags) {
  return {
    ...
    data: {
      ...
      sideEffects: [
        "ask the control-plane API to delete managed Worker, R2, and D1 resources for this deployment",
      ...
      notDoneLocally: [
        ...
        "no local wrangler r2 bucket delete"
      ],
      nextSteps: deploymentId
...
function previewSmoke(flags, value) {
  ...
  if (!existsSync("scripts/smoke-http.mjs")) {
    return fail(
      "SMOKE_SCRIPT_MISSING",
      "No scripts/smoke-http.mjs in this template.",
      "Add an HTTP smoke script or verify the deployed URL manually in a browser."
    );
  }
...
async function main() {
  ...
  if (resource === "modules" && action === "list") {
    ...
    emit(
      {
        ...
        data: {
          id: action,
          message: "Read docs/llms.txt and docs/api-boundary.md for template/module guidance."
        }
      },
      (data) => `${data.message}\n`,
      flags
    ...
    emit(await deployPreview(flags), null, flags);
  } else if (resource === "deploy" && action === "provision") {
    emit(await deployProvision(value, flags), null, flags);
  } else if (resource === "deploy" && action === "migrate") {
    emit(await deployMigrate(value, flags), null, flags);
  } else if (resource === "deploy" && action === "upload-plan") {
    emit(await deployUploadPlan(value, flags), null, flags);
  } else if (resource === "deploy" && action === "upload") {
    emit(await deployUpload(value, flags), null, flags);
  } else if (resource === "deploy" && action === "status") {
    emit(await deployStatus(value, flags), null, flags);
  } else if (resource === "deploy" && action === "resources") {
    emit(await deployResources(value, flags), null, flags);
  } else if (resource === "deploy" && action === "logs") {
    emit(await deployLogs(value, flags), null, flags);
  } else if (resource === "deploy" && action === "activate") {
    emit(await deployActivate(value, flags), null, flags);
  } else if (resource === "deploy" && action === "disable") {
    emit(await deployDisable(value, flags), null, flags);
  } else if (resource === "deploy" && action === "cleanup") {
    emit(await deployCleanup(value, flags), null, flags);
