...
## StackSuite Accounting And Commerce Parity Findings
- The old accounting/commerce port plan had stale open items: `accounting-core` OpenAPI/manifest parity, AP posting, AR sync for manual/Stripe/recurring flows, bank matching, accounting-core UI expansion, WooCommerce HMAC verification, contact snapshots, reservation/release, shipment batches, and printable packing slips are now implemented in modules/templates and guarded by template checks.
- The focused accounting template now has a stronger close-the-books workflow: invoice issue posts AR/revenue, recurring auto-issued invoices use the same GL/AR sync path, customer-payment applications post cash/AR settlement, Stripe payment-link webhooks settle through AR, reports expose AR aging/AP aging/open receivables/customer statements, and `/app/ledger` exposes fiscal periods, journal draft creation, posting, void/reversal, and trial balance.
- The focused commerce template now carries order-intake-to-fulfillment behavior: WooCommerce page sync normalizes/maps provider records, signed order webhooks and the import bridge create customer/product/sales-order/inventory state, sales-order lifecycle changes reserve/release stock, combo products reserve component stock, invoice handoff releases order reservations, and shipment completion deducts on-hand stock.
- The remaining StackSuite accounting/commerce work is mainly template cleanup and operational hardening: prune inherited ERP shell dependencies/migrations only after shared routes are intentionally narrowed, and add explicit Code Memory/CLI donor-scan docs only if future generated apps need built-in reuse guidance.
...
- `support-ticket` is now hardened with the remaining HelpGrid ticket primitives: per-tenant ticket numbers, comments/internal notes, attachment metadata, public follow-up share tokens, public token snapshots, and scoped wrappers. Pending upload sessions, raw R2 uploads, signed URLs, AI analyses/follow-up questions, billing tokens, WhatsApp, and UI stay outside the module boundary.
...
- Accounting settings parity now includes the active posting defaults with immediate behavior: AR/AP/income persisted defaults, general payment deposit default, and source-compatible Stripe deposit override. The donor's retained earnings, rounding, FX gain/loss, next-number counters, and AP approval threshold fields remain future slices because the inspected donor source either treats them as schema/design-only or because the local modules already own separate numbering/approval patterns.
- Accounts Payable vendor default expense parity is an implementation-backed donor slice: `accounting-system` stores `vendors.default_expense_account_id`, exposes it in vendor create/edit UI, and preselects bill line accounts from a preselected vendor default. `invoice-system-bao` does not contribute AP/vendor default behavior for this slice. The local AP module already had persistence, so the gap was behavioral/template wiring: bill and recurring bill line accounts now fall back to the vendor default when blank, while explicit line accounts still win and the accounting template validates defaults against active non-header expense accounts.
- AP bill detail parity is now implemented as a conservative read-only route proof: the AP module exports `getBill`, and the accounting template links `/app/payables/[id]` from the bill ledger with vendor/account labels, line items, totals, status, journal entry, lifecycle, and memo context. Donor bill detail also includes approval/post/void actions and payment history; those remain separate future phases because current local `markBillPayable` combines approval and posting and AP payment history/listing needs a dedicated payment-workbench surface.
- AP payment workbench parity now has read coverage: the module supports multi-bill partial applications plus `getBillPayment`/`listBillPayments` read APIs, `/app/payables/[id]` renders read-only bill-scoped payment history, and `/app/payables/payments/[id]` shows payment, vendor, account, journal, application, and lifecycle context. Void/reversal workflow remains future work because payment reversal needs accounting-core-safe journal reversal design.
- AP vendor detail parity is now implemented as a read-only route proof: the AP module exports `getVendor`, and the accounting template links `/app/payables/vendors/[id]` from bill rows, recurring bill rows, and a vendor directory card. The route shows profile/defaults, bills, payments, recurring schedules, aging, and 1099 readiness. Donor-backed vendor update/deactivate/reactivate and formal 1099 report APIs are now implemented. Payment void/reversal should still wait because current AP-to-accounting side effects need a dedicated reversal contract.
- AP vendor master completion is now implemented: `updateVendor`, `updateVendorStatus`, and `get1099VendorReport({ tenantId, year })` are module APIs with memory/D1 parity. The accounting template now captures phone, address, tax ID, 1099 flag, default terms, notes, and active state, and shows a current-year 1099 readiness card. The report intentionally provides readiness/payment totals and missing-tax-ID warnings only; it does not file tax forms or validate TINs.
- AP payment detail parity is now implemented as a read-only template route backed by the AP module payment read API. Bill and vendor payment-history rows link to `/app/payables/payments/[id]`, which keeps applications and lifecycle context visible without exposing void/reversal actions.
- AP approval/post split is now implemented: `markBillPayable` can approve without posting, `postBillToAccounting` posts approved unpaid bills, accounting-backed payments reject unposted bills, and the focused Payables UI exposes separate Approve, Post, and Pay states. The AP accounting poster now reuses existing posted source-ref journal entries for bill/payment retries, reducing duplicate-journal risk before void/reversal work.
- AP unpaid bill void is now implemented as a domain-level module/template slice: `voidBill` voids only unpaid, unposted bills, emits `accounts-payable.bill_voided`, and the accounting bill detail route exposes only the guarded void action. Subagent review confirmed the next AP priority should be payment void/reversal, using accounting-core reversal semantics rather than donor direct journal voiding.
...
- `html-renderer` is now implemented as a contract-checked utility module with HTML render documents, slug validation, TTL expiry, asset metadata, resolve, soft delete, and D1/memory stores. Public route auth, KV/R2 object storage, and large binary uploads remain app-adapter concerns.
- `content-cms` is now implemented as a contract-checked headless CMS module with content types, field definitions, versioned entries, publish/archive lifecycle, entry snapshots, localizations, locales, media metadata, and D1/memory stores. Auth, billing, setup, API keys, tenant quotas, AI page design, R2 uploads, CDN policy, and public rendering remain outside the module boundary.
- `video-generation` is now implemented as a contract-checked provider-neutral module with async jobs, provider task ids, reference metadata, normalized status reconciliation, idempotent provider URL output records, app-owned output attachment, cancellation, listing, and D1/memory stores. Provider credentials, billing/credits, auth, moderation, rate limits, webhooks, R2 byte downloads, signed URLs, browser FFmpeg, and story UI remain outside the module boundary.
- `project-progress` is now implemented from the EPMIS donor as a contract-checked module with tenant-scoped projects, worker access grants, progress logs, media metadata, comments, public access-token snapshots, and D1/memory stores. Auth/users/customers, QR image generation, byte uploads, R2 signing, email logs, and UI remain outside the module boundary.
- Markdown-to-PDF is not module material right now: the donor app is local drafts, browser preview, and `html2pdf.js` export. A future `document-renderer` should wait until multiple invoice/quote/content workflows need shared server-side PDF render jobs.
- QR Generator is browser-only in StackSuite, so it should stay a reference UI/free-tool candidate unless a template needs durable QR asset records.
- OpenClaw launcher/messenger, WhatsApp platform, CoreOps, PPT monitor, and Deepwork currently read as reference material rather than immediate module/template work: mine control-plane, channel-provider, revenue-distribution, document-monitoring, and focus-workflow patterns only when an existing roadmap surface needs them.
- Remaining utility apps such as web builder and app screen designer should stay P2/reference UI candidates until the System Harness quickstart and focused templates are cleaner.
