...
function deploymentInput(flags, environment = "preview") {
  ...
  return {
    templateId: manifest.id ?? config.template ?? "dot-ai-os",
    modules: moduleIds,
    config: {
      ...config,
      appName: appSlug,
      appSlug,
      template: config.template ?? manifest.id ?? "dot-ai-os"
    },
    environment,
...
function usage() {
  return `dot-ai-os microservices commands:
...
function usageAll() {
  return `dot-ai-os microservices commands (full):
