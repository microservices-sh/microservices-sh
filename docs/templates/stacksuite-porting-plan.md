...
- Banking now exposes imports, auto-detected mapping, mapping presets, import detail review, match suggestions, match creation, unmatch/exclude/restore correction actions, reconciliation start, reconciliation completion, and reconciliation detail review in the accounting template workflow. Remaining backlog is CSV preview, duplicate review, clear/unclear operations, richer summaries/history, and provider/OCR hooks.
...
- CSV field mapping auto-detection/presets, import stats, and duplicate detection. Auto-detection and presets are implemented; preview and duplicate review remain future work.
...
- clear/unclear, summary/history, and balance update workflows. Remove/exclude/restore correction actions are implemented; keep clear/unclear separate because current reconciliation completion owns the reconciled state.
...
6. Banking clear/unclear routes once bank-reconciliation exposes a separate cleared-state contract; unmatch/exclude/restore corrections are implemented on the Banking operator route.
