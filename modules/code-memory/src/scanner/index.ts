import type {
  ...
  SourceFileHint,
  SuggestedLogicCapsuleScanSummary,
  SuggestLogicCapsulesInput,
  SuggestLogicCapsulesResult
} from "../types";
...
const HEURISTICS: CapsuleHeuristic[] = [
  {
...
function normalizeFile(file: SourceFileHint): NormalizedFile | null {
...
function scoreFile(file: NormalizedFile, heuristic: CapsuleHeuristic): number {
...
function relatedTests(primaryPath: string, tests: NormalizedFile[], heuristic: CapsuleHeuristic): string[] {
...
function candidateForHeuristic(sourceId: string, files: NormalizedFile[], tests: NormalizedFile[], heuristic: CapsuleHeuristic): CreateLogicCapsuleInput | null {
  const matches = files
...
export function suggestLogicCapsulesFromFiles(input: SuggestLogicCapsulesInput): SuggestLogicCapsulesResult {
  ...
  const normalized = input.files.map(normalizeFile).filter((file): file is NormalizedFile => Boolean(file)).sort((a, b) => a.path.localeCompare(b.path));
  const sourceFiles = normalized.filter((file) => !file.isTest);
  const testFiles = normalized.filter((file) => file.isTest);
  const allCandidates = HEURISTICS.map((heuristic) => candidateForHeuristic(sourceId, sourceFiles, testFiles, heuristic)).filter(
    (candidate): candidate is CreateLogicCapsuleInput => Boolean(candidate)
  );
  const candidates = allCandidates.slice(0, maxCandidates);
  const scanSummary: SuggestedLogicCapsuleScanSummary = {
