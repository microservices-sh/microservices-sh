...
const appId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "booking-sveltekit";
const appDisplayName = cleanString(manifest.displayName) ?? "Booking SvelteKit";
...
function resolveSetupTarget(id) {
  ...
  const templateId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "booking-sveltekit";
