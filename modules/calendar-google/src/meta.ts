...
// Build a Result envelope Meta for a calendar-google use-case. correlationId is
// threaded from the caller (edge/dispatcher) when present, otherwise minted.
// See Plan 25 §4.
export function calendarGoogleMeta(deps?: { correlationId?: string; now?: () => number }): Meta {
  return withMeta({
