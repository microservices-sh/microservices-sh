...
Use `getVendor` for tenant-scoped vendor profile and 1099 metadata before building vendor-specific reports. Use `listBillPayments` for read-only AP payment history by tenant, vendor, bill, or status. Use `getBillPayment` when an operator needs the full payment record with applications before investigating journal references or a future void/reversal workflow.
