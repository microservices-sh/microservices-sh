...
// Build a Result envelope Meta for a billing-subscriptions use-case. correlationId
// is threaded from the caller (edge/dispatcher/webhook) when present, otherwise
// minted. See Plan 25 §4.
export function billingSubscriptionsMeta(deps?: { correlationId?: string; now?: () => number }): Meta {
  return withMeta({
