<script lang="ts">
  ...
  // Live-derive the slug from the workspace name until the user edits it directly.
  $effect(() => {
...
<svelte:head>
  <title>Set up your workspace · DOT AI OS</title>
</svelte:head>
...
<main class="setup">
  <div class="setup-card">
    <header class="setup-head">
      ...
      <p class="setup-lead">One operator workspace, one owner — that's you. Takes under a minute.</p>
    </header>
        ...
        {#if step === 1}
          <section class="setup-panel">
            ...
            <Field label="Your work email" id="email">
              <input id="email" type="email" autocomplete="email" bind:value={email} placeholder="you@example.com" />
            </Field>
            <Field label="Workspace name" id="orgName">
              <input id="orgName" maxlength="120" bind:value={orgName} placeholder="Acme Studio" />
            </Field>
            <Field label="Workspace URL" id="slug">
              <input id="slug" bind:value={slug} oninput={() => (slugEdited = true)} placeholder="acme" />
            </Field>
            <p class="setup-hint">Your workspace will live at <code>/{slug || "your-workspace"}</code>. You become the owner.</p>
          </section>
        {/if}
              ...
              <ul class="setup-invites">
                {#each invites as invite, i (i)}
                  <li class="setup-invite">
                    <input class="setup-invite-email" type="email" placeholder="teammate@example.com" bind:value={invite.email} aria-label="Teammate email" />
        ...
        {#if step === 3}
          <section class="setup-panel">
            ...
            <dl class="setup-review">
              <div><dt>Workspace</dt><dd>{orgName || "—"}</dd></div>
