...
const appId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "saas-starter-sveltekit";
const appDisplayName = cleanString(manifest.displayName) ?? "SaaS Starter SvelteKit";
...
function resolveSetupTarget(id) {
  ...
  const templateId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "saas-starter-sveltekit";
