...
const appId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "__MS_TEMPLATE_ID__";
const appDisplayName = cleanString(manifest.displayName) ?? "__MS_TEMPLATE_NAME__";
...
function resolveSetupTarget(id) {
  ...
  const templateId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "__MS_TEMPLATE_ID__";
