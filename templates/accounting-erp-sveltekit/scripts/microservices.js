...
const appId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "accounting-erp-sveltekit";
const appDisplayName = cleanString(manifest.displayName) ?? "Accounting ERP SvelteKit";
...
function resolveSetupTarget(id) {
  ...
  const templateId = cleanString(manifest.id) ?? cleanString(lock.template) ?? "accounting-erp-sveltekit";
