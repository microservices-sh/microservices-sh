...
- [x] P4 Admin calendar + polish — `/admin/calendar` week-agenda grid (TZ-aware, prev/this/next nav); `/admin/bookings` search + status filter (server-side) with TZ times; inline lucide-style `Icon.svelte` (no dep); Calendar nav link. Hardened the availability engine to degrade gracefully on missing/unmigrated tables (rules/exceptions/holds) instead of 500-ing the public booking page. Verified: build, check, dev screenshots (/book engine, calendar grid, bookings filter).
