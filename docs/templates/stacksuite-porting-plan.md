...
- Banking now exposes imports, auto-detected mapping, mapping presets, CSV preview/dedup review, import detail review, match suggestions, match creation, unmatch/exclude/restore correction actions, reconciliation start, provisional clear/unclear operations, reconciliation completion, and reconciliation detail review in the accounting template workflow. Remaining backlog is richer summaries/history and provider/OCR hooks.
...
- CSV field mapping auto-detection/presets, import stats, preview, and duplicate detection. Auto-detection, presets, preview, and duplicate review are implemented.
...
- clear/unclear, summary/history, and balance update workflows. Remove/exclude/restore correction actions and provisional clear/unclear operations are implemented; completion owns final reconciled state.
...
6. Richer banking summaries/history and provider/OCR hooks after the separate cleared-state contract; unmatch/exclude/restore corrections and clear/unclear operations are implemented on the Banking operator route.
