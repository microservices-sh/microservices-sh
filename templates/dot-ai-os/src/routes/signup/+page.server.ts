...
import { rememberWorkspaceOrg, loadWorkspaceContext } from "$lib/server/org-context";
...
// One-time workspace setup. DOT AI OS is NOT a public tenant funnel: it creates
// the operator workspace org and makes the first user its owner.
// After setup, /app routes employees straight into the shell.
export const load: PageServerLoad = async ({ locals, cookies }) => {
  // A signed-in employee who ALREADY belongs to a workspace doesn't need setup —
  // send them into the shell. But a signed-in user WITHOUT an operator workspace org is
  // exactly who this page is for: the /app onboarding CTA ("Set up your workspace")
...
export const actions: Actions = {
  default: async ({ request, cookies, locals, platform }) => {
    ...
    if (!orgName) {
      return fail(400, { error: "Enter a workspace name.", values });
    }
    ...
    // The org-team-rbac use case creates the operator workspace org, seeds owner/admin/member
    ...
    if (!result.ok) {
      return fail(result.status, { error: result.error?.message ?? "Could not create the workspace.", values });
    }
    ...
    rememberWorkspaceOrg(cookies, orgId);
