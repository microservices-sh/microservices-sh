...
<svelte:head>
  <title>Settings · DOT AI OS</title>
</svelte:head>

<main class="section">
  <Eyebrow>Organization settings</Eyebrow>
  <h1>{data.org.name}</h1>
  <p>Workspace settings, your effective permissions, and recent activity.</p>

  {#if form?.renamed}
    <Alert tone="success">Workspace details updated.</Alert>
  {:else if form?.error}
    ...
    <Card>
      ...
      {#if data.canManage}
        <form method="POST" action="?/rename" class="rename">
          <Field label="Workspace name" id="name">
