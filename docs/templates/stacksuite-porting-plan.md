...
- Banking now exposes imports, mapping presets, import detail review, match suggestions, match creation, unmatch/exclude/restore correction actions, reconciliation start, reconciliation completion, and reconciliation detail review in the accounting template workflow. Remaining backlog is CSV auto-detection/preview, duplicate review, clear/unclear operations, richer summaries/history, and provider/OCR hooks.
...
- CSV field mapping presets, import stats, and duplicate detection. Presets are implemented; auto-detection and duplicate review remain future work.
