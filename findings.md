...
- AP payment workbench parity now has read coverage: the module supports multi-bill partial applications plus `getBillPayment`/`listBillPayments` read APIs, and `/app/payables/[id]` renders read-only bill-scoped payment history. A dedicated payment detail page and void/reversal workflow remain future work because payment reversal needs accounting-core-safe journal reversal design.
