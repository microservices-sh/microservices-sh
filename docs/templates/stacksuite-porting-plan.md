...
- Banking now exposes imports, import detail review, match suggestions, match creation, unmatch/exclude/restore correction actions, reconciliation start, reconciliation completion, and reconciliation detail review in the accounting template workflow. Remaining backlog is richer CSV field mapping presets, duplicate review, clear/unclear operations, richer summaries/history, and provider/OCR hooks.
