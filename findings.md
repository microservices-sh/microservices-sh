...
- AP payment workbench parity is now partially implemented at the action/UI level: the module already supported multi-bill partial applications, and `/app/payables` now exposes a Record payment workbench that posts selected payable/partial bills with editable amounts plus method/reference/memo. AP payment read surfaces are still missing by design: future `getBillPayment`/`listBillPayments` store methods should power payment detail pages and bill-detail payment history.
