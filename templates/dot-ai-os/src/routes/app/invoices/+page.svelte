<script lang="ts">
  ...
  // ── Record-payment state (per open work-packet row) ────────────────────────
...
<svelte:head>
  <title>Work packets · DOT AI OS</title>
</svelte:head>

<main class="section">
  <Eyebrow>Work ledger</Eyebrow>
  <h1>Work packets</h1>
  <p>Issue and track scoped work/value packets for your workspace, powered by the invoice module.</p>

  {#if form?.paymentRecorded}
    <Alert tone="success">{form.paid ? "Payment recorded — work packet marked paid." : "Payment recorded."}</Alert>
  {:else if form?.number}
    <Alert tone="success">Work packet {form.number} issued.</Alert>
  {:else if form?.error}
  ...
  <div class="content-grid mt-6">
    <Card>
      ...
      {:else}
        <p class="empty">No work packets yet. Issue your first one -></p>
      {/if}
    </Card>
    ...
    {#if data.canManage}
      <Card>
        ...
        {#if data.customers.length === 0}
          <p class="empty">
            Add a contact first, then come back to issue a work packet.
            <a class="empty-cta" href="/app/customers">Go to Contacts -></a>
          </p>
        {:else}
          <form
            method="POST"
          ...
          >
            <Field label="Contact" id="customerId">
            ...
            <Button type="submit" variant="primary" disabled={!canSubmit}>
              {submitting ? "Issuing..." : "Issue work packet"}
            </Button>
          </form>
        {/if}
      </Card>
