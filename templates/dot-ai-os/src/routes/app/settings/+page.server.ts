...
export const load: PageServerLoad = async ({ locals, parent }) => {
  ...
  return {
    ...
    user: locals.user,
    permissions,
    // org.manage (admins) or "*" (owner) may rename the workspace.
...
export const actions: Actions = {
  // Rename the workspace / change its slug via the org-team-rbac use case
  ...
  rename: async ({ request, locals, cookies, parent }) => {
    ...
    if (!name) return fail(400, { error: "Enter a workspace name.", values });
    ...
    if (!result.ok || !result.data) {
      return fail(result.status ?? 400, { error: result.error?.message ?? "Could not update the workspace.", values });
    }
