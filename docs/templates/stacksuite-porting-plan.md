...
- Accounting depth has moved past the original thin slice. Ledger posting, fiscal period close/reopen/lock rules, fiscal period type/close-actor metadata, GAAP/IFRS setup seed selection, base-currency chart setup, post/void/reversal workflows, trial balance, AR/AP posting adapters, invoice/customer-payment settlement, Stripe settlement, and recurring-invoice job posting are implemented in the template/module split. Remaining accounting backlog is default-account setup persistence, richer account metadata, fiscal close concurrency hardening, and full financial statements.
...
1. Accounting setup route: default account persistence and full setup orchestration after chart standard, base currency, and fiscal year period generation.
...
- accounting default-account setup persistence, fiscal close concurrency hardening, and full statement contracts.
...
- accounting default-account setup persistence, fiscal close concurrency hardening, and full statement routes.
